package io.leavesfly.jimi.graph.parser;
...
public class JavaASTParser {
     ...
     * @return 解析结果
     */
    public io.leavesfly.jimi.graph.parser.ParseResult parseFile(Path filePath, Path projectRoot) {
        io.leavesfly.jimi.graph.parser.ParseResult result = io.leavesfly.jimi.graph.parser.ParseResult.builder()
        ...
        try {
            ...
            if (!parseResult.isSuccessful() || !parseResult.getResult().isPresent()) {
                ...
                return io.leavesfly.jimi.graph.parser.ParseResult.failure(
                    result.getFilePath(), 
                    "Parse failed");
            }
    ...
    private void parseTypeDeclaration(TypeDeclaration<?> typeDecl, String packageName, 
                                     CodeEntity fileEntity, io.leavesfly.jimi.graph.parser.ParseResult result) {
    ...
    private void parseInheritance(ClassOrInterfaceDeclaration classDecl, CodeEntity classEntity,
                                  io.leavesfly.jimi.graph.parser.ParseResult result) {
        // 解析继承 (extends)
    ...
    private void parseConstructor(ConstructorDeclaration constructor, CodeEntity classEntity, 
                                  String classQualifiedName, io.leavesfly.jimi.graph.parser.ParseResult result) {
