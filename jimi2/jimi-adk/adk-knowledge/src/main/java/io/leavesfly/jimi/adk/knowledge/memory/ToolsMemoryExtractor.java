 ...
 */
@Slf4j
public class ToolsMemoryExtractor {

    private final MemoryManager memoryManager;
    private final MemoryConfig config;

    public ToolsMemoryExtractor(MemoryManager memoryManager, MemoryConfig config) {
    ...
    public Mono<Void> extractFromToolResult(ToolResult result, String toolName) {
        ...
        // 只处理成功的结果
        if (!result.isOk() || result.getMessage() == null || result.getMessage().isEmpty()) {
            return Mono.empty();
        }
        ...
        return Mono.defer(() -> {
            ...
            // 提取和摘要内容
            String content = summarize(result.getMessage(), 200);
    ...
    private double calculateConfidence(String toolName, ToolResult result) {
        ...
        // 根据输出长度调整（太短可能信息不足）
        int outputLength = result.getMessage().length();
        if (outputLength < 50) {
            confidence *= 0.8;
