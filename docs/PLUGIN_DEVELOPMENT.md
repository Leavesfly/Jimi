...
**即时 enable/disable**：`SkillRegistry` 提供 `unregister` 接口，`/plugin disable` 对 Skills 即时生效。
...
**会话级注入**：`McpModuleAdapter` 发现插件 `mcp/` 目录下的配置文件后，`JimiFactory` 在创建 Engine 时会自动调用 `PluginRegistry.getAllDiscoveredMcpConfigFiles()` 合并所有插件的 MCP 配置，一次性传给 `MCPToolProvider`。因此插件声明的 MCP Server 在**会话创建时自动生效**。

**⚠️ 运行时限制**：由于 `MCPToolProvider` 的 `mcpConfigFiles` 在会话创建时固定，`/plugin reload` 后新增的 MCP 配置需**重启会话**才能生效。

### 4.5 Subagent

支持两种格式：**Jimi 原生 YAML 格式**和 **Claude Code Markdown 格式**。

#### 格式一：Jimi 原生（agent.yaml + 外部 prompt 文件）
...
#### 格式二：Claude Code 标准（agent.md，frontmatter + body）
...
agents/
└── code-reviewer/
    └── agent.md                  # frontmatter 配置 + Markdown body（system prompt）
```
...
**Claude Code 格式特性**：
- frontmatter 中的 `---` 分隔符之间是 YAML 配置
- frontmatter 之后的 Markdown body 直接作为 system prompt（支持 `${JIMI_WORK_DIR}` 等模板变量替换）
- `tools` 支持逗号分隔字符串（`tools: Read, Glob, Grep`）和 YAML 列表两种写法
- `disallowedTools` 是 `exclude_tools` 的 Claude Code 标准别名
- `maxTurns` 限制 Agent 最大交互轮次
- `description` 是 Claude Code 标准必需字段，用于 SubAgentTool 工具描述与自动委派

**即时注入**：两种格式都支持即时 enable/disable。`AgentModuleAdapter` 在加载时解析 `agent.yaml` 或 `agent.md` 并调用 `AgentRegistry.registerAgentSpec()` 写入 `specCache`，使插件贡献的 Agent 在当前会话即时可用。`/plugin disable` 时通过 `AgentRegistry.unregisterAgentSpec()` 从缓存驱逐，即时回退。
...
### Q3: `/plugin disable` 后哪些扩展点即时失效？

所有五种扩展点均支持即时 enable/disable：

- ✅ Skills（`SkillRegistry.unregister`）
- ✅ Hooks（`HookRegistry.unregisterHook`）
- ✅ Commands（`CustomCommandRegistry.unregisterCommand`）
- ✅ Agents（`AgentRegistry.unregisterAgentSpec`）
- ℹ️ MCP（会话级注入，reload 需重启会话生效）
...
**文档版本**：对齐 P0+P1+P2（MVP 完整）+ Skills/MCP/Agents 注入优化
**最后更新**：2026-06-30
